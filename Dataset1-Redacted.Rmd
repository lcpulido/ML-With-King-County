...
### We have houses in the set worth over $7 million
...
###Response: price
###Predictors: All predictors except...

```{r}
library(randomForest)
house.price <- randomForest(price~.-, data=houses, mtry=3, importance=TRUE, na.action=na.omit)
house.price
```
...
I wanted to use a neural network. I decided to use the mean as my metric for the new column I created because I was more intrested in the average price of a house in this set. Here I now use avgprice I created before splitting the data into train and test sets. 
...
```{r}
library(neuralnet)
#nn1 <- neuralnet(price~, data=train)
#plot(nn1)

housenet <- neuralnet(avgprice~, train, hidden=1, lifesign="minimal", linear.output=FALSE, threshold=0.1)
```
